...
public final class Renderer {
    ...
    private void keepFrame(@NonNull final List<RendererPlugin> plugins, @NonNull final RenderingContext rc,
                           @NonNull final FrameWriter frameWriter, @Nullable final BufferedImage bi, final int frame,
        ...
        if (bi != null && keepFrame != null && keepFrame > 0) {
            drawWaypoints(bi, frame, wpMap);
            final var marker = drawMarker(bi, frame);

            for (final var plugin : plugins) {
                plugin.renderFrame(frame, marker, bi);
            }
